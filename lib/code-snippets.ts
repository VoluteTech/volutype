...
const codeSnippets: Record<CodeLanguage, CodeSnippet[]> = {
  javascript: [
    ...
    { lines: ["class Counter {", "  constructor() { this.count = 0; }", "  increment() { this.count++; }", "  getCount() { return this.count; }", "}"] },
    { lines: ["const debounce = (fn, delay) => {", "  let timeout;", "  return (...args) => {", "    clearTimeout(timeout);", "    timeout = setTimeout(() => fn(...args), delay);", "  };", "};"] },
    { lines: ["const users = [", "  { name: 'Alice', score: 95 },", "  { name: 'Bob', score: 87 },", "  { name: 'Charlie', score: 92 }", "];", "const avgScore = users.reduce((sum, u) => sum + u.score, 0) / users.length;"] },
  ],
  python: [
    ...
    { lines: ["from functools import reduce", "numbers = [1, 2, 3, 4, 5]", "total = reduce(lambda x, y: x + y, numbers)", "print(total)"] },
    { lines: ["def quicksort(arr):", "    if len(arr) <= 1:", "        return arr", "    pivot = arr[len(arr) // 2]", "    left = [x for x in arr if x < pivot]", "    middle = [x for x in arr if x == pivot]", "    right = [x for x in arr if x > pivot]", "    return quicksort(left) + middle + quicksort(right)"] },
  ],
  c: [
    ...
    { lines: ["#include <string.h>", "", "int main() {", "    char str1[] = \"Hello\";", "    char str2[] = \"World\";", "    strcat(str1, \" \");", "    strcat(str1, str2);", "    printf(\"%s\\n\", str1);", "    return 0;", "}"] },
    ...
    { lines: ["#define MAX 100", "", "int main() {", "    int arr[MAX] = {5, 10, 15, 20, 25};", "    int size = 5;", "    int sum = 0;", "    for (int i = 0; i < size; i++) {", "        sum += arr[i];", "    }", "    printf(\"Sum: %d\\n\", sum);", "    return 0;", "}"] },
  ],
  cpp: [
    ...
    { lines: ["#include <iostream>", "#include <memory>", "using namespace std;", "", "class MyClass {", "public:", "    void sayHello() { cout << \"Hello!\" << endl; }", "};", "", "int main() {", "    unique_ptr<MyClass> obj = make_unique<MyClass>();", "    obj->sayHello();", "    return 0;", "}"] },
    { lines: ["#include <iostream>", "using namespace std;", "", "int main() {", "    int arr[] = {64, 34, 25, 12, 22, 11, 90};", "    int n = sizeof(arr) / sizeof(arr[0]);", "    for (int i = 0; i < n - 1; i++) {", "        for (int j = 0; j < n - i - 1; j++) {", "            if (arr[j] > arr[j + 1]) {", "                swap(arr[j], arr[j + 1]);", "            }", "        }", "    }", "    return 0;", "}"] },
  ],
  go: [
    ...
    { lines: ["package main", "", "import \"fmt\"", "", "func main() {", "    ch := make(chan string, 2)", "    ch <- \"Hello\"", "    ch <- \"World\"", "    close(ch)", "    for msg := range ch {", "        fmt.Println(msg)", "    }", "}"] },
  ],
  rust: [
    ...
    { lines: ["fn main() {", "    let strings = vec![\"apple\", \"banana\", \"cherry\"];", "    let sorted: Vec<&str> = strings.iter().cloned().collect();", "    for s in sorted {", "        println!(\"{}\", s);", "    }", "}"] },
    { lines: ["trait Greeter {", "    fn greet(&self);", "}", "", "struct Hello;", "", "impl Greeter for Hello {", "    fn greet(&self) {", "        println!(\"Hello, World!\");", "    }", "}", "", "fn main() {", "    let h = Hello;", "    h.greet();", "}"] },
  ],
  php: [
    ...
    { lines: ["<?php", "$users = [", "    ['name' => 'Alice', 'score' => 95],", "    ['name' => 'Bob', 'score' => 87],", "    ['name' => 'Charlie', 'score' => 92],", "];", "$names = array_column($users, 'name');", "print_r($names);", "?>"] },
    { lines: ["<?php", "function fibonacci($n) {", "    if ($n <= 1) return $n;", "    return fibonacci($n - 1) + fibonacci($n - 2);", "}", "", "for ($i = 0; $i < 10; $i++) {", "    echo fibonacci($i) . \" \";", "}", "?>"] },
    { lines: ["<?php", "$data = ['id' => 1, 'name' => 'Test', 'active' => true];", "['id' => $id, 'name' => $name] = $data;", "echo \"$id, $name\";", "?>"] },
  ],
  zig: [
  ...
  java: [
    ...
    { lines: ["import java.util.List;", "import java.util.ArrayList;", "", "public class Main {", "    public static void main(String[] args) {", "        List<String> names = new ArrayList<>();", "        names.add(\"Alice\");", "        names.add(\"Bob\");", "        names.add(\"Charlie\");", "        names.forEach(System.out::println);", "    }", "}"] },
  ],
  csharp: [
